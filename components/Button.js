...
const Button = ({ text, setDisplay, display, equation, setEquation }) => {
  // handleClick function handles the calculations and sets the display 
  // whenever a button is clicked on the MyCustomWidget component
  const handleClick = () => {
    if (display === "0" && "+-x÷%+/-.".includes(text)) {
      ...
      // In case of an equal button click, evaluate the equation until now
